  ...
  <section class="section">
    <div class="container">
      <div class="cake-page-heading">
        <h1>Bánh kem</h1>
        <p>Những mẫu bánh kem nhận đặt và một vài mẫu có sẵn để bạn tham khảo. Bạn có thể nhấp vào từng bánh để xem kỹ hơn hình ảnh, thông tin chi tiết.</p>
      </div>

      <form class="cake-search-form cake-search-form--live" data-cake-client-search-form>
        <label class="screen-reader-text" for="cake-search-banh-kem">Tìm bánh kem</label>
        <input
          id="cake-search-banh-kem"
          type="search"
      ...
      <div class="cake-filter-chips" aria-label="Lọc bánh kem" data-cake-filter-group>
        ...
        <button type="button" data-cake-filter="nhan-dat-truoc">Đặt trước</button>
      </div>

      <div class="cards">
        <?php
        $banh_kem_query = new WP_Query([
          'post_type'      => 'cake',
          'posts_per_page' => -1,
          'meta_query'     => [
            [
        ...
        if (!empty($banh_kem_query->posts)) {
          $priority_posts = [];
          $normal_posts   = [];

          foreach ($banh_kem_query->posts as $cake_post) {
          ...
          $banh_kem_query->posts = array_merge($priority_posts, $normal_posts);
        }
        ...
        <?php
          ...
          echo '<p>Tiệm đang chuẩn bị thêm các mẫu bánh kem mới, bạn ghé lại sau hoặc nhắn tiệm để được tư vấn nhanh nhé.</p>';
        endif;
        ?>
      </div>


      <p class="cake-search-empty" data-cake-search-empty hidden>Không có bánh kem phù hợp. Bạn thử nhập từ khóa khác nhé.</p>
